# < BREAST CANCER PREDICTION TOOL FOR WOMEN IN UROMI EDO STATE NIGERIA >
# -*- IHON OSEGHALE & MIKEL OSEI -*-
...
def init():
    ...
    print("\nTrain accuracy: {} %".format(
        100 - np.mean(np.abs(Y_prediction_train - train_y.T)) * 100))

    print("\nTest accuracy: {} %".format(
        100 - np.mean(np.abs(Y_prediction_test - test_y.T)) * 100))
...
def model(Xtrain, Ytrain, num_of_iterations, alpha):
    ...
    parameters, grads, costs = optimize(
        Xtrain, Ytrain, w, b, num_of_iterations, alpha)
